...
void CPPT10Ctrl::Close(BOOL bQuit) 
{
...
BOOL CPPT10Ctrl::OpenPresToWin(CString strPath, HWND hWnd, int nWidth, int nHeight)
{
	...
	_Slide10 slide = m_slides.Add(1, 1);	// Show the first slide of the presentation
...
BOOL CPPT10Ctrl::AppendSlide()
{
	_Slide10 slide = m_slides.Add(m_slides.GetCount() + 1, 1);	
	return TRUE;
}

BOOL CPPT10Ctrl::GetRunSlideAndGetPPTHandle()
{
	HWND hWndArray[10];
	int i = 0;
	for (i = 0; i < 10; i++)
...
BOOL CALLBACK CPPT10Ctrl::EnumWindowsProc(HWND hWnd, LPARAM lParam)
{
...
BOOL CPPT10Ctrl::OpenPPT(CString strPath)
{
	m_app.Activate();
...
BOOL CPPT10Ctrl::OpenPresentation(CString strPath)
{
	m_app.Activate();
    m_app.SetVisible((long)TRUE);
...
void CPPT10Ctrl::ShowPPT(BOOL bShow)
{
	if (m_hWndPPT)
	{
...
void CPPT10Ctrl::ClosePresentation(BOOL bQuit) 
{
	// Check to see whether we've started the server.
	if(m_app.m_lpDispatch == NULL)
		return;
...
BOOL CPPT10Ctrl::SaveAsJPG(CString strDir)
{
	m_presentation.SaveAs(strDir, 17, 0);
	return TRUE;
}

//BOOL CPPT10Ctrl::WebPublish(CString strDir)
BOOL CPPT10Ctrl::WebPublish(CString strPath, CString strPPTPath, CString strDir)
{
//	WebOptions10 wo;
...
//	PublishObjects10 pos;
//	pos = m_presentation.GetPublishObjects();

//	PublishObject10 po = pos.Item(1);
...
BOOL CPPT10Ctrl::PrevSlide()
{
	m_pView.Previous();
	return TRUE;
}

BOOL CPPT10Ctrl::NextSlide()
{
	m_pView.Next();
	return TRUE;
}

BOOL CPPT10Ctrl::GotoSlide(int n)
{
...
int CPPT10Ctrl::GetCurrentSlideNumber()
{
...
BOOL CPPT10Ctrl::FirstSlide()
{
	GotoSlide(1);
	return TRUE;
}

BOOL CPPT10Ctrl::LastSlide()
{
	GotoSlide(GetSlideCount());
	return TRUE;
}

int CPPT10Ctrl::GetSlideCount()
{
	return m_slides.GetCount();
}

BOOL CPPT10Ctrl::SaveMedia(CString strDir)
{
	CStringArray strArrayContent;
	PageSetup10 ps = m_presentation.GetPageSetup();
	...
	{
		_Slide10 slide = m_slides.Item(COleVariant((long)i));
		Shapes10 shapes = slide.GetShapes();
		int nShapeCount = shapes.GetCount();
		strSlide.Format("%d", i);
		bHasMedia = FALSE;
		for (int j = 1; j <= nShapeCount; j++)
		{
			Shape10 shape = shapes.Item(COleVariant((long)j));
...
BOOL CPPT10Ctrl::GetSliderTitle(CStringArray& strArray)
{
	strArray.RemoveAll();
	int nSlideCount = GetSlideCount();
	for (int i = 1; i <= nSlideCount; i++)
	{
		...
		{
			Shape10 title = shapes.GetTitle();
			if (title.GetHasTextFrame())
			{
				TextFrame10 txFrame = title.GetTextFrame();
				TextRange10 txRange = txFrame.GetTextRange();
...
BOOL CPPT10Ctrl::SaveFile(LPCTSTR strFileName, CStringArray& strArray, BOOL bNoEmpty)
{ 
	CFile cf;
...
BOOL CPPT10Ctrl::SetPublishtoDir()
{
	WebOptions10 wo = m_presentation.GetWebOptions();
...
BOOL CPPT10Ctrl::SetThumb(CString strDir, int SlideCnt)
{
	FILE *stream_in;
	char* pline;
...
void CPPT10Ctrl::deleteDirectory(CString directory_path)
{
	CFileFind finder;
    CString path;
...
int CPPT10Ctrl::GetCurrentAnimaNumber()
{
	return m_pView.GetClickIndex();
}

void CPPT10Ctrl::PumpMessage()
{
	MSG msg;
