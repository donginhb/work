...
BOOL CProgressWnd::Create(CWnd* pParent, LPCTSTR pszTitle, BOOL bSmooth /* = FALSE */)
{
    ...
    bSuccess = CreateEx(WS_EX_DLGMODALFRAME | WS_EX_TOPMOST, // Extended style
                        ...
                        pszTitle,                          // Title
                        WS_OVERLAPPED | WS_BORDER | WS_CAPTION,     // style
...
void CProgressWnd::SetWindowSize(int nNumTextLines, int nWindowWidth /*=390*/)
{
    ...
    if (pDC) 
	{
        ...
        CancelSize = pDC->GetTextExtent(m_strCancelLabel + _T("  ")) +
                                        CSize(EdgeSize.cx * 8, EdgeSize.cy * 5);
    ...
    if (m_bCanCancel)
		ProgressRect.SetRect(TextRect.left, CancelRect.top, 
			                 CancelRect.left - nMargin, CancelRect.bottom);
	else
		ProgressRect.SetRect(TextRect.left, CancelRect.top, 
			                 CancelRect.right, CancelRect.bottom);
...
int CProgressWnd::SetPos(int nPos)                    
{
    ...
    if (m_nMaxValue > m_nMinValue)
        nPercentage = nPercentage = ((nPos - m_nMinValue) * 100) / 
...
void CProgressWnd::SetText(LPCTSTR fmt, ...)
{
    ...
    va_list args;
    TCHAR* pbuffer = new char[512];

    va_start(args, fmt);
    _vstprintf(pbuffer, fmt, args);
    va_end(args);

    m_Text.SetWindowText(pbuffer);

	delete [] pbuffer;
}

void CProgressWnd::SetText(UINT nID)
{
	CString StrText;
