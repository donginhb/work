	...
	m_bTrackSelection(FALSE)
{
	...
	m_strDefaultText = str;

	str.LoadString(IDS_MORE_COLOR);
	m_strCustomText = str;
}
...
LONG CColorButton::OnSelEndOK(UINT lParam, LONG /*wParam*/)
{
    ...
    if (OldColor != m_Color)
        if (pParent) pParent->SendMessage(CPN_SELCHANGE, GetDlgCtrlID(), m_Color);
...
BOOL CColorButton::OnClicked()
{
    ...
    if (pParent)
        pParent->SendMessage(CPN_DROPDOWN, (WPARAM)GetDlgCtrlID(), m_Color);
