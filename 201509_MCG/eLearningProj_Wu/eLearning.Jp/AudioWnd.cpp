// AudioWnd.cpp : implementation file
//
...
BEGIN_MESSAGE_MAP(CAudioWnd, CWnd)
	//{{AFX_MSG_MAP(CAudioWnd)
	ON_WM_ERASEBKGND()
	ON_WM_MOUSEMOVE()
	ON_WM_LBUTTONDOWN()
	ON_WM_LBUTTONUP()
...
/////////////////////////////////////////////////////////////////////////////
// CAudioWnd message handlers

void CAudioWnd::DrawWave(CDC* pDC)
{

	CString strCourse = MACRO_MAINFRM->GetCourse();
	CString strThumbDir1 = MACRO_HTML_DIR(strCourse)+"THUMB3";
	if (!::FileExists(strThumbDir1 + "\\AudioThumb.wav"))
		return;
	CFile wavFile(strThumbDir1 + "\\AudioThumb.wav", CFile::modeRead | CFile::shareDenyNone);
	long fileLen = wavFile.GetLength() - 46;
	wavFile.Seek(46, CFile::begin);
	long scale = 10;
	long count = m_lLength / scale;
	long step = 0;
	if (fileLen > count)
	{
		step = fileLen / count / 2;
	}
	short *data = new short[count];
	for (int i = 0; i < count; i++)
	{
		byte tempData[2];
		wavFile.Read(tempData, sizeof(byte) * 2);
		data[i] = *(short*)tempData;
		wavFile.Seek(step * 2, CFile::current);
	}
	wavFile.Close();

	CRect rcClient;
	GetClientRect(rcClient);
	float gap = (float)(rcClient.bottom - rcClient.top) / 2;
	float middle = (float)(rcClient.bottom + rcClient.top) / 2;
	CPen penWhite(PS_SOLID, 1, ::GetSysColor(COLOR_3DHILIGHT));
	CPen* pOldPen = pDC->SelectObject(&penWhite);
	pDC->MoveTo(rcClient.left, middle);
	pDC->LineTo(rcClient.left + TimeToPos(count * scale), middle);
	for (int i = 0; i < count; i++)
	{
		short value = data[i];
		if (value < 0)
			value = -value;
		float height = (float)value * gap / 32767;
		if(TimeToPos(i * scale) > rcClient.right)
			break;
		int left = rcClient.left + TimeToPos(i * scale);
		pDC->MoveTo(left, middle + height);
		pDC->LineTo(left, middle - height);
		static int left0 = 0;
		if (left - left0 >= 1)
		{
			int here = 0;
...
BOOL CAudioWnd::OnEraseBkgnd(CDC* pOrigDC) 
{
	...
	CBrush br(RGB(144, 144, 144));//RGB(132, 130, 132));//::GetSysColor(COLOR_3DFACE));
...
BOOL CAudioWnd::IsTimeVisible(long lTime)
{
	CRect rcClient;
	GetClientRect(rcClient);
...
BOOL CAudioWnd::Create(DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID)  
{
	if (m_strClass == "")
...
BOOL CAudioWnd::CreateEx(LPCTSTR lpszClassName, LPCTSTR lpszWindowName,
...
int CAudioWnd::TimeToPos(long lTime)
{
	return (lTime - m_lTimeStart) * m_nPixelUnit / m_nUnit;
}

long CAudioWnd::PosToTime(int xPos)
{
	long lRet = m_lTimeStart + m_nUnit * xPos / m_nPixelUnit;
	return lRet;
}

void CAudioWnd::SetPixelUnit(int lTime)
{
	m_nPixelUnit = lTime;
}

void CAudioWnd::SetTimeStart(long lTime)
{
	m_lTimeStart = lTime;
}

void CAudioWnd::SetTimeUnit(int nTimeUnit)
{
	m_nUnit = nTimeUnit;
}

void CAudioWnd::Initialize()
{
}

BOOL CAudioWnd::AddSegment(long lTimeStart, long lTimeEnd, CString strParam, 
...
void CAudioWnd::GetSegString(CString& str)
{
	str = "";
	for (int i = 0; i < m_pSegArray->GetSize(); i++)
	{
...
void CAudioWnd::SetSegArray(CStringArray& strArrayVideo, int nType)
{
	//if (m_pSegArray.GetSize()!=NULL)
	  //::ReleaseObArray(m_pSegArray);
	for (int i = 0 ; i < m_pSegArray->GetSize();)
	{
		CVideoEffectSeg* p_Seg = (CVideoEffectSeg*)m_pSegArray->GetAt(i);
		if (p_Seg->m_nType == nType)
...
void CAudioWnd::OnMouseMove(UINT nFlags, CPoint point) 
{
	if (m_bTracking)
	{
		m_ptPrePos = point;
...
CRect CAudioWnd::GetSegRect(int nIndex) 
{
	CRect rcClient;
	GetClientRect(rcClient);
...
void CAudioWnd::OnLButtonDown(UINT nFlags, CPoint point) 
{
	if (m_bTracking)
	{
...
void CAudioWnd::OnLButtonUp(UINT nFlags, CPoint point) 
{
	if (!m_bTracking)
	{
	...
	CVideoEffectSeg* pSeg = (CVideoEffectSeg*)m_pSegArray->GetAt(m_nClickCtrl);
	int nCtrlID = pSeg->m_nType == 1 ? ID_AUDIO_REPLACE : ID_AUDIO_MIX;
	int nCount = 0;
	CVideoEffectSeg* pSegTmp;
	for (int i = 0; i <= m_nClickCtrl; i++)
	{
		pSegTmp = (CVideoEffectSeg*)m_pSegArray->GetAt(i);
		if (pSegTmp->m_nType == pSeg->m_nType)
		{
			nCount++;
		}
	}
	...
	if (m_bFirstMove)
	{
		...
		if (!TimeOverlap(lTimeStart, lTimeEnd, m_nClickCtrl))
		{
			...
			GetParent()->SendMessage(WM_PERIOD_TOBE_MOVED, nCount, nCtrlID);

			pSeg->m_lTimeStart = lTimeStart;
			pSeg->m_lTimeEnd = lTimeEnd;

			GetParent()->SendMessage(WM_PERIOD_MOVED, m_nClickCtrl, GetDlgCtrlID());
			GetParent()->SendMessage(WM_PERIOD_MOVED, nCount, nCtrlID);
...
LRESULT CAudioWnd::OnMouseLeave(WPARAM wparam, LPARAM lparam)
{
	m_bLeaveTrack = FALSE;
	for (int i = 0; i < m_pSegArray->GetSize(); i++)
	{
...
void CAudioWnd::EnterCtrl(int nCtrl)
{
	if (nCtrl < 0)
		return;
...
void CAudioWnd::LeaveCtrl(int nCtrl)
{
	if (nCtrl < 0)
		return;
...
int CAudioWnd::CtrlEntered(CPoint point, int* pnType)
{
...
void CAudioWnd::OnInvertTracker(const CRect& rect)
{
	CRect rc = rect;
	ClientToScreen(rc);
	::DrawResizeBar(rc);
...
void CAudioWnd::DeletePeriodByIndex(int nIndex)
{
...
BOOL CAudioWnd::TimeOverlap(long lTimeStart, long lTimeEnd, int nIndexNotCare)
{
	for (int i = 0; i < m_pSegArray->GetSize(); i++)
	{
		if (i == nIndexNotCare)
...
void CAudioWnd::SetSegTip(int nIndex, CString strTip) 
{
	if (nIndex < 0 || nIndex >= m_pSegArray->GetSize())
		return;
...
void CAudioWnd::SetSegColor(int nIndex, COLORREF crNormal, COLORREF crHot) 
{
	if (nIndex < 0 || nIndex >= m_pSegArray->GetSize())
		return;
...
void CAudioWnd::GetSegInfo(int nIndex, long& lTimeStart, long& lTimeEnd, 
...
void CAudioWnd::OnRButtonUp(UINT nFlags, CPoint point) 
{
#ifdef _BUNDLE
	...
	if (nCmd == ID_DEL_OBJ)
	{
		...
		//GetParent()->PostMessage(WM_PERIOD_DELETE, nCount, nCtrlID);
		//m_pSegArray->RemoveAt(nCtrl);
		GetParent()->PostMessage(WM_PERIOD_DELETE, nCtrl, nCtrlID);
		//RedrawWindow();
	}
	else if (nCmd == ID_VIEW_OBJ)
	{
		GetParent()->PostMessage(WM_PERIOD_VIEW, nCtrl, GetDlgCtrlID());
		GetParent()->PostMessage(WM_PERIOD_VIEW, nCount, nCtrlID);
	}
	else if (nCmd == ID_EDIT_CONTENT)
	{
		GetParent()->PostMessage(WM_PERIOD_EDIT, nCtrl, GetDlgCtrlID());
		GetParent()->PostMessage(WM_PERIOD_EDIT, nCount, nCtrlID);
	}
	
	CWnd::OnRButtonUp(nFlags, point);
}

void CAudioWnd::DeletePeriod(long lTimeStart, long lTimeEnd)
{
	for (int i = 0; i < m_pSegArray->GetSize(); i++)
	{
		CVideoEffectSeg* pSeg = (CVideoEffectSeg*)m_pSegArray->GetAt(i);
...
BOOL CAudioWnd::IsAnyPeriodDeleted(long lTimeStart, long lTimeEnd)
{
	for (int i = 0; i < m_pSegArray->GetSize(); i++)
	{
		CVideoEffectSeg* pSeg = (CVideoEffectSeg*)m_pSegArray->GetAt(i);
...
void CAudioWnd::InsertPeriod(long lTimeInsert, long lDuration)
{
	for (int i = 0; i < m_pSegArray->GetSize(); i++)
	{
		CVideoEffectSeg* pSeg = (CVideoEffectSeg*)m_pSegArray->GetAt(i);
...
CString CAudioWnd::GetTimeStr(long nPos)
{
	CString strTime = ::ConvertTimeToStr(nPos);
	return strTime;
}

void CAudioWnd::AddToolTip()
{
	if (m_pToolTip )
	  delete m_pToolTip;
	m_pToolTip = new CToolTipCtrl;
...
LRESULT CAudioWnd::WindowProc(UINT message, WPARAM wParam, LPARAM lParam) 
{
	switch (message)
    {
		case WM_LBUTTONDOWN:
...
int CAudioWnd::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
...
void CAudioWnd::ResetTooltip()
{
	AddToolTip();
}

void CAudioWnd::OnSize(UINT nType, int cx, int cy) 
{
...
void CAudioWnd::ShowToolTip(long lTimeStart, long lTimeEnd)
{
	CPoint pt;
	GetCursorPos(&pt);
	int npx = pt.x + 16;
