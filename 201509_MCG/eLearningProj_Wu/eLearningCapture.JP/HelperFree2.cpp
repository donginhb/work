...
{	
	...
	BOOL fOK = (cbBuffer==cf.ReadHuge(pBuffer, cbBuffer));
	if(fOK) {
...
// dump all freehand drawunits
	...
	for(ii=0; ii<DrawUnits.GetSize(); ii++) {
		...
		cst.Format("    if(g_time>%d) fh_main_stop();", punit->lTimeMax - punit->lTimeMin); cst += szLF;
		cf.Write((LPSTR)(LPCSTR)cst, cst.GetLength());
		...
		while(jj<punit->lPointCount) {
			...
			if(punit->Points[jj].lTime<=lTimeBase) {
				cst.Format("%0.4f,%0.4f", (double)punit->Points[jj].xx/punit->lWidth, (double)punit->Points[jj].yy/punit->lHeight);
		...
		cst = "        document.DrawUtility.DrawFreeHand(g_usepen, g_param);"; cst += szLF;
		cf.Write((LPSTR)(LPCSTR)cst, cst.GetLength());
...
// dump fh_main_start
	cst.Format("function fh_main_start(id) {");	cst += szLF;
	cf.Write((LPSTR)(LPCSTR)cst, cst.GetLength());
