...
//	INSSBuffer *pNSSBuffer=NULL;
//	HRESULT hr = m_pWriterBase->m_pWriter->AllocateSample(m_pWavReader->pwfx->nAvgBytesPerSec, &pNSSBuffer);
//	if(FAILED(hr))
//		return FALSE;
//
//	pNSSBuffer->SetLength(m_pWavReader->pwfx->nAvgBytesPerSec);
//
//	LPBYTE pWavBuffer=NULL;
//	pNSSBuffer->GetBuffer(&pWavBuffer);
...
//			DWORD dwWavSize = dwmsDeliverSize * m_pWavReader->pwfx->nAvgBytesPerSec / 1000;

//			UINT uRetSize=0;
//			WavReaderReadDirectData(m_pWavReader, m_uPosWave, dwWavSize, pWavBuffer, &uRetSize);
//
//			QWORD qwTime = m_qwSegmentTime + (QWORD)dwmsTime * 10000 - m_qwStartTime;
//			hr = m_pWriterBase->m_pWriter->WriteSample(m_pWriterBase->m_dwInputAudio, qwTime, 0, pNSSBuffer);
...
// to add this period seems to be strange but indeed, it makes better results.
	...
	if(dwOutputNum==m_dwOutputAudio) {
		...
		hr = m_pWriterBase->m_pWriter->WriteSample(m_pWriterBase->m_dwInputAudio, qwWaveTime, dwFlags, pSample);
		if(FAILED(hr)) return hr;
