...
// No actual reason for seek has bugs so it is just a dirty fix!!
	if(!Run_AutoMode(m_qwStartTime+1500000, m_qwStopTime-m_qwStartTime))
		return FALSE;
							 ...
							 DWORD dwFlags, INSSBuffer *pSample, void *pvContext) 
{
	...
	DWORD dwNewLength = dwLength / m_pWavWriter->pwfx->nBlockAlign * m_pWavWriter->pwfx->nBlockAlign;
