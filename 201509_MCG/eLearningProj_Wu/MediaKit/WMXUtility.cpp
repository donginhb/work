...
STDMETHODIMP CWMXUtility::MultiWMXAudio2Wav(BSTR bstrAudioList, IUnknown *pManager, BSTR bstrMixerTimers, long lBitsPerSample, long lChannels, long lSampleRate, BSTR bstrOutput)
{
	...
	CString cstOutput(bstrOutput);
	CString cstTemp(cstOutput + "_tmp.wav");
	DeleteFile(cstOutput);
	DeleteFile(cstTemp);
...
STDMETHODIMP CWMXUtility::MultiWMXMixer2Wav(BSTR bstrWav, BSTR bstrLists, BSTR bstrOutWav)
{
	...
	UINT uPosWave=0;	
	for(int ii=0; ii<TimeArray.GetSize(); ii++) {
		UINT uTemp = (UINT)(TimeArray.GetAt(ii) / 1000.0 * WavReader.pwfx->nAvgBytesPerSec);
		uTemp = uTemp / WavReader.pwfx->nBlockAlign * WavReader.pwfx->nBlockAlign;		
		if(uPosWave < uTemp) {
...
STDMETHODIMP CWMXUtility::MultiWMXDirectAppend(BSTR bstrWMXNameTable, VARIANT_BOOL vbVideo, VARIANT_BOOL vbScript, BSTR bstrOutWMX)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState())
	...
	for(int ii=0; ii<CstArray.GetSize(); ii++) {	
		...
		cwmxdumper.SetUserClock(TRUE);
		cwmxdumper.SetSelectedStreams(TRUE, (vbVideo==OATRUE)&&cwriterbase.m_fVideoExisted, (vbScript==OATRUE)&&cwriterbase.m_fScriptExisted);
...
STDMETHODIMP CWMXUtility::MultiWMXOutput(BSTR bstrMain, BSTR bstrAudioReplace, BSTR bstrAudioMix, IUnknown *pManager, BSTR bstrProfile, BSTR bstrOutput)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState())

	return MultiWMXExport(bstrMain, NULL, pManager, bstrProfile, bstrOutput);
/*
	CDWordArray MainTimeArray, MainStartArray, MainStopArray;
	CStringArray MainCstArray;
	if(!ParseList(bstrMain, MainTimeArray, MainCstArray, MainStartArray, MainStopArray))
		return E_FAIL;

	CDWordArray ReplaceTimeArray, ReplaceStartArray, ReplaceStopArray;
	CStringArray ReplaceCstArray;
	if(!ParseList(bstrAudioReplace, ReplaceTimeArray, ReplaceCstArray, ReplaceStartArray, ReplaceStopArray))
		return E_FAIL;

	CDWordArray MixTimeArray, MixStartArray, MixStopArray;
	CStringArray MixCstArray;
	if(!ParseList(bstrAudioMix, MixTimeArray, MixCstArray, MixStartArray, MixStopArray))
		return E_FAIL;
	
	int ii=0;
	for(ii=0; ii<MainTimeArray.GetSize(); ii++) {
		CComBSTR cbstrWMX(MainCstArray.GetAt(ii));
		CComBSTR cbstrOutput(MainCstArray.GetAt(ii) + ".wmv");
		HRESULT hr = WMXProcess(cbstrWMX, bstrProfile, pManager,
			MainTimeArray.GetAt(ii), MainStartArray.GetAt(ii), MainStopArray.GetAt(ii),
			ReplaceCstArray, ReplaceTimeArray, ReplaceStartArray, ReplaceStopArray,
			MixCstArray, MixTimeArray, MixStartArray, MixStopArray,
			cbstrOutput);
		if(FAILED(hr))
			return hr;
	}
	...
	CString cstTable;
	for(ii=0; ii<MainCstArray.GetSize(); ii++) {		
		cstTable += MainCstArray.GetAt(ii);
		cstTable += ".wmv;";
	}
	
	CComBSTR cbstrTable(cstTable);
	return MultiWMXDirectAppend(cbstrTable, OATRUE, OATRUE, bstrOutput);
*/

}

STDMETHODIMP CWMXUtility::WMXAudio2Wav(BSTR bstrWMX, long lmsStart, long lmsStop, long lBitsPerSample, long lChannels, long lSampleRate, BSTR bstrOutWav)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState())

	CString cstOutput(bstrOutWav);
	DeleteFile(cstOutput);
	
	WAVWRITER WavWriter;
	memset(&WavWriter, 0, sizeof(WavWriter));
	if(!WavWriterCreate_PCM(&WavWriter, (WORD)lBitsPerSample, (WORD)lChannels, (DWORD)lSampleRate, (LPSTR)(LPCSTR)cstOutput))
		return E_FAIL;
	...
	if(!WMX2Wav(bstrWMX, lmsStart, lmsStop, &WavWriter)) {
		fError = TRUE;
	}
...
STDMETHODIMP CWMXUtility::WMXDirectTrim(BSTR bstrWMX, long lmsStart, long lmsStop, VARIANT_BOOL vbVideo, VARIANT_BOOL vbScript, BSTR bstrOutput)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState())

	CWMXDumper	cdumper;
	CComBSTR	cbstrSource(bstrWMX);
	if(!cdumper.Open(cbstrSource)) return E_FAIL;

	cdumper.ConfigAudioOutputCompressed();
	cdumper.ConfigVideoOutputCompressed();
	cdumper.ConfigScriptOutputCompressed();

	CWriterBase cwriterbase;
	BOOL fOK = cwriterbase.Create(bstrOutput, cdumper.m_pProfile);
	if(!fOK) return E_FAIL;
	...
	BOOL bOK = FALSE;
	cdumper.SetWriterBase(&cwriterbase);

	cdumper.SetUserClock(TRUE);
	cdumper.SetSelectedStreams(TRUE, (vbVideo==OATRUE)&&cwriterbase.m_fVideoExisted, (vbScript==OATRUE)&&cwriterbase.m_fScriptExisted);
	
	QWORD qwStart = (QWORD)lmsStart * 10000, qwStop = (QWORD)lmsStop * 10000;
	bOK = cdumper.Run_AutoMode(qwStart, qwStop - qwStart);
...
STDMETHODIMP CWMXUtility::WMXConvertTrim(BSTR bstrWMX, long lmsStart, long lmsStop, VARIANT_BOOL vbVideo, VARIANT_BOOL vbScript, BSTR bstrProfile, BSTR bstrOutput)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState())
	...
	CWriterBase cwriterbase;
	BOOL fOK = cwriterbase.Create(bstrOutput, bstrProfile);
	if(!fOK) return E_FAIL;
	
	WM_MEDIA_TYPE *pwmt=NULL;

	pwmt = cdumper.GetAudioOutputFormat();
	if(pwmt) {
		cwriterbase.ConfigAudioInputFormat(pwmt);
		cdumper.FreeFormatAllocated(pwmt); pwmt=NULL;
	}
	pwmt = cdumper.GetVideoOutputFormat();
	if(pwmt) {
		cwriterbase.ConfigVideoInputFormat(pwmt);
		cdumper.FreeFormatAllocated(pwmt); pwmt=NULL;
	}	
...
#include "MBRPrx.h"
STDMETHODIMP CWMXUtility::MBRPrxGen(long lA1kbps, long lV1kbps, long lwV1, long lhV1, long lA2kbps, long lV2kbps, long lwV2, long lhV2, long lA3kbps, long lV3kbps, long lwV3, long lhV3, BSTR bstrPrxOut)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState())
		
	return GenMBRPrx(lA1kbps, lV1kbps, lwV1, lhV1, lA2kbps, lV2kbps, lwV2, lhV2, lA3kbps, lV3kbps, lwV3, lhV3, bstrPrxOut);
}

STDMETHODIMP CWMXUtility::MultiWMXExport(BSTR bstrList, IUnknown *pAudioManager, IUnknown *pVideoManager, BSTR bstrProfile, BSTR bstrOutput)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState())

	CStringArray NameArray;
	CDWordArray TimeArray, StartArray, StopArray;
	if(!ParseList(bstrList, TimeArray, NameArray, StartArray, StopArray))
		return E_FAIL;

	HRESULT hr = 0;
	int ii=0;
	CString cstTempTable, cstTemp;
	QWORD qwLen=0;
	for(ii=0; ii<NameArray.GetSize();ii++) {
		cstTemp.Format("%s.tmp%d.wmv", NameArray.GetAt(ii), ii);
		CComBSTR bstrTemp(NameArray.GetAt(ii)), bstrTempOut(cstTemp);
/* Use convert case, sound would be stranged in some trim
		if((pAudioManager!=NULL)||(pVideoManager!=NULL))
			hr = WMXConvertByDS(bstrTemp, pAudioManager, pVideoManager, StartArray.GetAt(ii), StopArray.GetAt(ii), bstrProfile, bstrTempOut);
		else 
*/
		hr = WMXDirectTrim(bstrTemp, StartArray.GetAt(ii), StopArray.GetAt(ii), OATRUE, OATRUE, bstrTempOut);
		qwLen += (QWORD)10000 * (StopArray.GetAt(ii) - StartArray.GetAt(ii));
		
		if(FAILED(hr))
			return hr;
		if(ii!=0) cstTempTable += ";";
		cstTempTable += cstTemp;
	}
	
	CComBSTR bstrTempTable(cstTempTable);
	CComBSTR bstrTmpOutput(CString(bstrOutput)+"tmp.wmv");
	hr = MultiWMXDirectAppend(bstrTempTable, OATRUE, OATRUE, bstrTmpOutput);
	...
	// hr = WMXConvertMix(bstrTmpOutput, pAudioManager, pVideoManager, NULL);
	hr = WMXConvertByDS(bstrTmpOutput, pAudioManager, pVideoManager, bstrProfile, bstrOutput);

#ifndef _DEBUG
	DeleteFile(CString(bstrTmpOutput));
#endif
	
	return hr;
	
/* Audio strange if use this code!!	It seems that use WMF 7 to output WMF 9 buggy when audio part!! Should use D-Show!!
	CWMXDumper	cdumper;
	if(!cdumper.Open(bstrTmpOutput)) return E_FAIL;
	
	CWriterBase cwriterbase;
	BOOL fOK = cwriterbase.Create(bstrOutput, bstrProfile);
	if(!fOK) return E_FAIL;

cdumper.ConfigAudioOutputSimpleUncompressed(16, 2, 22050);
cdumper.ConfigVideoOutputSimpleUncompressed(24, BI_RGB);
cwriterbase.ConfigAudioInputSimpleUncompressed(16, 2, 22050);
cwriterbase.ConfigVideoInputSimpleUncompressed(24, BI_RGB);
WM_MEDIA_TYPE *pwmt=NULL;
	...
	BOOL bOK = FALSE;	
	cdumper.SetWriterBase(&cwriterbase);
	cdumper.SetManager((IEasyManager*)pAudioManager, (IEasyManager*)pVideoManager); // lazy it COM
	
	cdumper.SetUserClock(TRUE);
	cdumper.SetSelectedStreams(TRUE, TRUE, TRUE);
	
	bOK = cdumper.Run_AutoMode(0, qwLen - 1);
...
/* // audio has bug if using the codes
	HRESULT hr = 0;
	QWORD	qwOffset=0;
	
	CWriterBase cwriterbase;	
	for(int ii=0; ii<NameArray.GetSize(); ii++) {	
		hr = E_FAIL;
		
		CWMXDumper	cwmxdumper;
		CComBSTR	cbstrSource(NameArray.GetAt(ii));
		if(!cwmxdumper.Open(cbstrSource)) 
			break;
		
		cwmxdumper.SetSelectedStreams(TRUE, TRUE, TRUE);
		
		cwmxdumper.ConfigAudioOutputSimpleUncompressed(16, 2, 22050);
		cwmxdumper.ConfigVideoOutputSimpleUncompressed(24, BI_RGB);
		cwmxdumper.ConfigScriptOutputCompressed();
		
		if(ii==0) {
			if(!cwriterbase.Create(bstrOutput, cwmxdumper.m_pProfile))				
				break;
		...
		CComBSTR cbstrType("TEXT");
		CString cstFile = NameArray.GetAt(ii);
		int nPtr = cstFile.ReverseFind('\\');
		if(nPtr<0) {
			int nPtr = cstFile.ReverseFind(':');
			if(nPtr<0) nPtr = -1;
		...
		cwmxdumper.SetUserClock(TRUE);
		cwmxdumper.SetSelectedStreams(TRUE, cwriterbase.m_fVideoExisted, cwriterbase.m_fScriptExisted);
		
		QWORD qwStart = (QWORD)10000 * StartArray.GetAt(ii), qwStop = (QWORD)10000 * StopArray.GetAt(ii);
		if(!cwmxdumper.Run_AutoMode(qwStart, qwStop - qwStart)) 
			break;
...
STDMETHODIMP CWMXUtility::WMXReplaceAudio_Wav(BSTR bstrFile, IUnknown *pManager, BSTR bstrReplace, BSTR bstrProfile, BSTR bstrOutput)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState())
		
	CWMXAudioReplaceWav WMXAudioReplaceWav;
	
	if(IsWindow(m_hWndProgress)) WMXAudioReplaceWav.SetupProgressHandler(m_hWndProgress, m_lMsgProgress);
	if(m_pbAbort!=NULL) WMXAudioReplaceWav.SetupQueryAbort(m_hWndAbort, m_pbAbort);
	
	if(!WMXAudioReplaceWav.MakeReplace(bstrFile, (IEasyManager*)pManager, bstrReplace, bstrProfile, bstrOutput))
		return E_FAIL;
	
	return S_OK;
}

STDMETHODIMP CWMXUtility::WMXAudioMixReplace2Wav(BSTR bstrWMX, BSTR bstrReplaceList, BSTR bstrMixList, long lBitsPerSample, long lChannels, long lSampleRate, BSTR bstrOutputWav)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState())
	...
	CDWordArray ReplaceTimeArray, ReplaceStartArray, ReplaceStopArray;
	CStringArray ReplaceFileArray;
	if(!ParseList(bstrReplaceList, ReplaceTimeArray, ReplaceFileArray, ReplaceStartArray, ReplaceStopArray))
		return E_FAIL;

	CDWordArray WavReplaceTimeArray, WavReplaceStartArray, WavReplaceStopArray;
	CStringArray WavReplaceFileArray;
	for(ii=0; ii<ReplaceFileArray.GetSize();ii++) {
		CString cst(ReplaceFileArray.GetAt(ii));
		cst += ".$$$50Replace.wav";
		hr = WMXAudio2Wav(CComBSTR(ReplaceFileArray.GetAt(ii)), ReplaceStartArray.GetAt(ii), ReplaceStopArray.GetAt(ii), lBitsPerSample, lChannels, lSampleRate, CComBSTR(cst));
		if(FAILED(hr)) return E_FAIL;
		WavReplaceTimeArray.Add(ReplaceTimeArray.GetAt(ii));
		WavReplaceFileArray.Add(cst);
		WavReplaceStartArray.Add(0);
		WavReplaceStopArray.Add(ReplaceStopArray.GetAt(ii)-ReplaceStartArray.GetAt(ii));
	}
	
	CDWordArray MixTimeArray, MixStartArray, MixStopArray;
	CStringArray MixFileArray, MixWaveArray;
	if(!ParseList(bstrMixList, MixTimeArray, MixFileArray, MixStartArray, MixStopArray))
		return E_FAIL;
	
	CDWordArray WavMixTimeArray, WavMixStartArray, WavMixStopArray;
	CStringArray WavMixFileArray;
	for(ii=0; ii<MixFileArray.GetSize();ii++) {
		CString cst(MixFileArray.GetAt(ii));
		cst += ".$$$51Mix.wav";
		hr = WMXAudio2Wav(CComBSTR(MixFileArray.GetAt(ii)), MixStartArray.GetAt(ii), MixStopArray.GetAt(ii), lBitsPerSample, lChannels, lSampleRate, CComBSTR(cst));
		if(FAILED(hr)) return E_FAIL;
		WavMixTimeArray.Add(MixTimeArray.GetAt(ii));
		WavMixFileArray.Add(cst);
		WavMixStartArray.Add(0);
		WavMixStopArray.Add(MixStopArray.GetAt(ii)-MixStartArray.GetAt(ii));
	}
	
	CString cstTmpWMXWav(bstrWMX);
	cstTmpWMXWav += ".$$$52Main.wav";

	double dLen=0;
	hr = GetWMXDuration(bstrWMX, &dLen);
	if(FAILED(hr)) return E_FAIL;

	hr = WMXAudio2Wav(bstrWMX, 0, (long)dLen, lBitsPerSample, lChannels, lSampleRate, CComBSTR(cstTmpWMXWav));
	if(FAILED(hr)) return E_FAIL;

	CString cstTmpReplaceWav(cstTmpWMXWav);
	cstTmpReplaceWav += ".$$$53Replace.wav";
	DeleteFile(cstTmpReplaceWav);
	if(!WavFileReplacer((LPSTR)(LPCSTR)cstTmpWMXWav, WavReplaceTimeArray, WavReplaceFileArray, WavReplaceStartArray, WavReplaceStopArray, (LPSTR)(LPCSTR)cstTmpReplaceWav))
		return E_FAIL;

#ifndef _DEBUG
	DeleteFile(cstTmpWMXWav);
	{
		for(int xx=0; xx<WavReplaceFileArray.GetSize();xx++)
			DeleteFile(WavReplaceFileArray.GetAt(xx));
	}
#endif

	CString cstFinalWav(bstrOutputWav);
	DeleteFile(cstFinalWav);
	if(!WavFileMixer((LPSTR)(LPCSTR)cstTmpReplaceWav, WavMixTimeArray, WavMixFileArray, WavMixStartArray, WavMixStopArray, (LPSTR)(LPCSTR)cstFinalWav))
		return E_FAIL;

#ifndef _DEBUG
	DeleteFile(cstTmpReplaceWav);
	{
		for(int xx=0; xx<WavMixFileArray.GetSize();xx++)
			DeleteFile(WavMixFileArray.GetAt(xx));
	}
#endif
