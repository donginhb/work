...
/*      This routine has to be called before WaveReadFile or Grouped WaveDirectReadFile as it searchs for the chunk to descend into for
	...
	Note:: any swapping from WaveDirectReadFile to WaveReadFile, should be called again!!! CCHUNG !!!*/
...
/*      This will read wave data from the wave file.  Makre sure we've called StartDataRead to get the data chunk info
	, else this will fail bigtime!!
	hmmioIn         - Handle to mmio.
	cStart			- Start Position to read
	cbRead          - # of bytes to read.   
	pbDest          - Destination buffer to put bytes.
	cbActualRead- # of bytes actually read.
*/

int WaveDirectReadFile(		
		HMMIO hmmioIn,                          // IN
		UINT cStart,								// IN
		UINT cbRead,                            // IN           
		BYTE *pbDest,                           // IN
		...
		)
{
	...
	if(cStart >= pckIn->cksize)
		return -1;

	cbDataIn = cbRead;
	if((cStart+cbDataIn) > pckIn->cksize)
		cbDataIn = pckIn->cksize - cStart;

	if((nError = mmioSeek(hmmioIn, pckIn->dwDataOffset + cStart, SEEK_SET)) == -1)
		goto ERROR_CANNOT_DIRECTREAD;

	if((nError = mmioRead(hmmioIn, pbDest, cbDataIn)) == -1)
		goto ERROR_CANNOT_DIRECTREAD;

	*cbActualRead = cbDataIn;

	goto FINISHED_DIRECTREADING;

ERROR_CANNOT_DIRECTREAD:
	*cbActualRead = 0;

FINISHED_DIRECTREADING:
