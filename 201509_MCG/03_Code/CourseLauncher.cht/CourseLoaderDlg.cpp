...
#include "..\elearning.cht\ColGrid.h"
...
#include "..\ComCapture\UserErrMsg.h"
...
{ 
	...
	for (i = 0; i < m_strArrayCourse.GetSize(); i++)
	{
									 ...
									 int nDest, int nCheckScorn)
{
	...
	::GetFiles(strSrcPath + "\\html\\videos", "wmv", strArrayFiles);
	for (int i = 0; i < strArrayFiles.GetSize(); i++)
		strArrayAllFiles.Add("html\\videos\\" + strArrayFiles[i]);
	strArrayFiles.RemoveAll();
	::GetFiles(strSrcPath + "\\html", "wmv", strArrayFiles);
	for (i = 0; i < strArrayFiles.GetSize(); i++)
											   ...
											   CString strDestPath1)
{
	...
	for (int j = 0; j < strArrayFiles.GetSize(); j++)
	{
		...
		BOOL bMainASF = (strFileName.CompareNoCase("default.wmv") == 0);
...
{
	strFile.MakeLower();
	if (strFile.Right(4) != ".wmv")
		return FALSE;

	if (strFile == "default.wmv")
		return TRUE;
...
void CCourseLoaderDlg::UpdateMediaFile(CString strSrcPath, CString strPath, CString strMediaServer)
{
	...
	//strArrayReplace.Add("var g_strServer=\"" + strMediaServer + "/\";");
	strArrayTag.Add("var g_strDefaultASF=\"videos/default.wmv\";");
	strArrayReplace.Add("var g_strDefaultASF=\"default.wmv\";");
	...
	::GetFiles(strSrcPath + "\\html", "wmv", strArrayASFFile);
	...
	for (i = 0; i < strArrayHTMFile.GetSize(); i++)
	{
